...
int lfs_spi_flash_init(struct lfs_config *cfg) {
    spi_flash_init();
    if (GD25Q40E_ID == spi_flash_read_id()) {
        ...
        cfg->block_size = GD25Q40E_SECTOR_SIZE;
        // 设备上可擦除块的数量，即容量
        cfg->block_count = GD25Q40E_SECTOR_NUM;
...
int lfs_spi_flash_read(const struct lfs_config *cfg, lfs_block_t block, lfs_off_t off, void *buffer, lfs_size_t size) {
    ...
    spi_flash_buffer_read( (uint8_t *)buffer,block * cfg->block_size + off, size);
    return LFS_ERR_OK;
}
...
int lfs_spi_flash_prog(const struct lfs_config *cfg, lfs_block_t block, lfs_off_t off, const void *buffer, lfs_size_t size) {
    ...
    spi_flash_buffer_write( (uint8_t *)buffer,block * cfg->block_size + off, size);
    return LFS_ERR_OK;
}
...
int lfs_spi_flash_erase(const struct lfs_config *cfg, lfs_block_t block) {
    ...
    LFS_ASSERT(block < cfg->block_count);
    spi_flash_write_enable();
    spi_flash_sector_erase(block * cfg->block_size);
    return LFS_ERR_OK;
}
